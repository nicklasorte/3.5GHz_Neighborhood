...
close all;
app=NaN(1);
folder1='C:\Users\nlasorte\OneDrive - National Telecommunications and Information Administration\MATLAB2024\3.5GHz Neighborhood Sims Rev3';
cd(folder1)
addpath(folder1)
pause(0.1)
...
toc;
%%%%%%Same data structure as the randomized real
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Z Drive
rev=3001; %%%%%%Just East: -8dB with 50% and 1 MC 
...
mc_size=1%1000; %%%%%Number of Monte Carlo Iterations (WinnForum 2000)
...
FreqMHz=3550;
confidence=50;
mc_percentile=100%95; %%%%95th Percentile
...
num_pts=8%2%%%%8;  %%%%%%%Number of DPA Sample Points along the front edge, Typically 8
number_rand_pts=NaN(1);
min_ant_loss=40; %%%%%%%%Main to side gain: 40dB
cell_expand_all_dpa(1:26,:)
cell_expand_all_dpa(32:45,:)
dpa_idx=find(matches(cell_expand_all_dpa(:,1),'East 1'))
min_azimuth=0;
max_azimuth=360;
sim_folder1='Z:\MATLAB\3.5GHz\Neighborhood Rev2'
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
cell_tf_ship0{1}=0;
%%%cell_dpa_geo(:,6)=cell_tf_ship1; %%%%%%%I forgot what this does.
cell_dpa_geo
...
fdr_dB=0
pol_mismatch=0
array_bs_eirp_reductions=(array_bs_eirp-bs_down_tilt_reduction-network_loading_reduction-fdr_dB-pol_mismatch) %%%%%Rural, Suburban, Urban cols:(1-3), No Mitigations/Mitigations rows:(1-2)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
